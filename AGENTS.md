...
Instructions for AI agents installing or modifying this stack.
...
That's it. The script is Mac-only (Apple Silicon), idempotent, and walks the user through 8 phases including dep installation, sister-repo cloning (`vllm-mlx` fork + `searxng-mcp` as siblings of `localclaude`), container setup, and verification. `./install.sh --help` for flags.
...
- **`coder-480` profile**: only useful if the user has at least 256 GB of RAM (M3 Ultra-class) OR has a remote Mac that does. Don't suggest `coder-480` to a 16/32/64 GB Mac user -- they should use `coder` (~17 GB), `coder-next` (~32 GB), or `qwen36` (~20 GB) instead.
...
- **`brew install claude`** -- that installs the Claude desktop GUI, not the CLI. The correct cask is `brew install --cask claude-code`.

## Modifying

If you're making changes:

- This repo (`akaszubski/localclaude`) ships `README.md`, `ARCHITECTURE.md`, `CHANGELOG.md`, `LICENSE`, `install.sh`, `AGENTS.md`, and the `localclaude` bash script. Sister components live in their own repos (`akaszubski/vllm-mlx` fork, `akaszubski/searxng-mcp`) -- don't try to vendor them.
- Pre-commit there are no hooks in this repo, but watch out for: literal `/Users/<name>/` paths, anything starting with `# claudeMd` in fixtures, hardcoded SSH endpoints (use env vars like `LOCALCLAUDE_CODER_480_REMOTE` instead).
- Architectural decisions live in `CHANGELOG.md` -- add an entry there when you change a default or land a new perf knob.
...
- For bugs in this repo (`localclaude` script, `install.sh`, docs): https://github.com/akaszubski/localclaude/issues
- For inference-server bugs: https://github.com/waybarrios/vllm-mlx/issues (issues enabled on upstream; the fork has issues disabled)
